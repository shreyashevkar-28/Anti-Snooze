...
function generateHamiltonianPath(size) {
  ...
  for (let attempt = 0; attempt < 200; attempt++) {
    ...
    while (path.length < total) {
      let nbrs = neighbors(r, c, size).filter(([nr,nc]) => !visited[nr*size+nc]);
...
function buildPuzzle(size, numWaypoints) {
  ...
  for (let i = 0; i < numWaypoints; i++) {
    if (i === 0) {
      ...
      const next = Math.round((i+1) * step);
  ...
  return {
    size,
    nodes: indices.map((solIdx, i) => ({ id: i + 1, r: sol[solIdx][0], c: sol[solIdx][1] })),
    solution: sol
  };
}
...
export default function ZipPuzzle({ difficulty, meta, onSolve }) {
  ...
  const cellSize = Math.min(46, Math.floor(250 / size));
  const GAP      = 3;
  const PAD      = 8;
  ...
  const startCell = (r, c) => {
    ...
    if (nodeId === 1) { setPath([[r,c]]); setDrawing(true); return; }
    const idx = path.findIndex(([pr,pc]) => pr===r && pc===c);
    if (idx !== -1) { setPath(path.slice(0, idx+1)); setDrawing(true); }
  };
  ...
  const enterCell = (r, c) => {
    ...
    if (path.length >= 2) {
      ...
      if (prev[0]===r && prev[1]===c) { setPath(p => p.slice(0,-1)); return; }
    }
    const idx = path.findIndex(([pr,pc]) => pr===r && pc===c);
    if (idx !== -1) { setPath(p => p.slice(0, idx+1)); return; }
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14, alignItems:"center" }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize:11, color:"#64748b", letterSpacing:"0.25em", fontWeight:700 }}>
        ...
        {nextReq <= nodes.length ? (
          <div style={{
            ...
            >{nextReq}</motion.div>
            <span style={{ fontSize:10, color:meta.color, letterSpacing:"0.2em", fontWeight:700 }}>
              NEXT
            </span>
          </div>
        ) : (
          <span style={{ fontSize:11, color:"#4ade80", letterSpacing:"0.2em", fontWeight:700 }}>
            {solved ? "✓ COMPLETE" : "FILL ALL CELLS"}
          </span>
        )}

        <span style={{ fontSize:11, color:"#64748b", letterSpacing:"0.25em", fontWeight:700, fontVariantNumeric:"tabular-nums" }}>
          {pct}%
        </span>
      </div>
      ...
      >
        ...
        <svg style={{
          ...
          {path.length > 1 && (
            <path d={svgD} stroke={meta.color} strokeWidth={cellSize * 0.56} strokeLinecap="round" strokeLinejoin="round" fill="none" opacity="0.5" />
          )}
          {path.length > 1 && (
            <path d={svgD} stroke={meta.color} strokeWidth={cellSize * 0.2} strokeLinecap="round" strokeLinejoin="round" fill="none" opacity="0.9" />
          )}
          {!solved && nextNode && (
            <motion.circle cx={cellXY(nextNode.r, nextNode.c).x} cy={cellXY(nextNode.r, nextNode.c).y} r={cellSize * 0.44} fill="none" stroke={meta.color} strokeWidth="2" opacity="0.5"
              animate={{ r:[cellSize*0.44, cellSize*0.55, cellSize*0.44], opacity:[0.5,1,0.5] }} transition={{ repeat:Infinity, duration:1.1 }}
            />
          )}
          {!solved && path.length > 0 && (
            <circle cx={cellXY(path[path.length-1][0], path[path.length-1][1]).x} cy={cellXY(path[path.length-1][0], path[path.length-1][1]).y} r={cellSize*0.22} fill="#fff" opacity="0.9" />
          )}
        </svg>
        ...
        {Array.from({length:size},(_,r)=>
          Array.from({length:size},(_,c)=>{
            ...
            return (
              <div key={key} onMouseDown={()=>startCell(r,c)} onMouseEnter={()=>enterCell(r,c)} onTouchStart={e=>{e.preventDefault(); startCell(r,c); setDrawing(true);}}
                style={{
                  position:"absolute", left: PAD + c*(cellSize+GAP), top:  PAD + r*(cellSize+GAP), width:cellSize, height:cellSize,
                  zIndex: nodeId ? 5 : 2, cursor:"crosshair", display:"flex", alignItems:"center", justifyContent:"center",
                }}
              >
                {nodeId && (
                  <motion.div initial={{ scale:0.7 }} animate={{ scale:1 }}
                    style={{
                      width: cellSize*0.64, height: cellSize*0.64, borderRadius:"50%",
                      background: inPath ? meta.color : "#111d30",
                      border:`2.5px solid ${inPath ? meta.color : isNextN ? meta.color : "#2a3d5a"}`,
                      display:"flex", alignItems:"center", justifyContent:"center",
                      boxShadow: inPath ? `0 0 14px ${meta.color}cc, 0 0 0 3px ${meta.color}33` : isNextN ? `0 0 12px ${meta.color}66` : "none",
                      transition:"all 0.2s", pointerEvents:"none",
                    }}
                  >
                    <span style={{ fontSize:Math.max(11, cellSize*0.34), fontWeight:900, color: inPath ? "#000" : "#94a3b8", fontFamily:"inherit", lineHeight:1, userSelect:"none" }}>
                      {nodeId}
                    </span>
                  </motion.div>
                )}
        ...
        {nodes.map((n, i) => {
          ...
          return (
            <div key={n.id} style={{ display:"flex", alignItems:"center", gap:4 }}>
              <motion.div animate={active ? { scale:[1,1.18,1] } : {}} transition={{ repeat:Infinity, duration:1 }}
                style={{
                  width:20, height:20, borderRadius:"50%",
              ...
              {i < nodes.length-1 && (
                <span style={{ color:done?meta.color:"#1e2d4a", fontSize:8, fontWeight:800 }}>→</span>
              )}
            </div>
          );
        })}
      ...
      <div style={{ display:"flex", gap:12 }}>
        <button onClick={reset} style={{ padding:"6px 14px", borderRadius:4, background:"transparent", border:"1px solid #1e2d4a", color:"#64748b", fontSize:10, letterSpacing:"0.3em", fontFamily:"inherit", fontWeight:700, cursor:"pointer" }}
          onMouseEnter={e=>{e.target.style.borderColor="#64748b";e.target.style.color="#94a3b8";}} onMouseLeave={e=>{e.target.style.borderColor="#1e2d4a";e.target.style.color="#64748b";}}
        >↺ RESET</button>

        <button onClick={newPuzzle} style={{ padding:"6px 14px", borderRadius:4, background:"transparent", border:`1px solid ${meta.border}`, color:meta.color, fontSize:10, letterSpacing:"0.3em", fontFamily:"inherit", fontWeight:700, cursor:"pointer" }}
          onMouseEnter={e=>{e.target.style.background=meta.bg;}} onMouseLeave={e=>{e.target.style.background="transparent";}}
        >⟳ NEW PUZZLE</button>
      </div>

      <p style={{ color:"#1e2d4a", fontSize:10, letterSpacing:"0.15em", fontWeight:600, margin:0, textAlign:"center" }}>
